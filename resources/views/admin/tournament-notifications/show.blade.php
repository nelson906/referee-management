...
<div class="container mx-auto px-4">
            ...
            <div class="bg-white rounded-lg shadow-md">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <ul class="space-y-2">
                                <li><strong>📧 Notifiche Inviate:</strong> {{ $tournamentNotification->sent_at ? $tournamentNotification->sent_at->format('d/m/Y H:i') : 'Mai inviate' }}</li>
                                <li><strong>👤 Inviato da:</strong> {{ $tournamentNotification->sentBy ? $tournamentNotification->sentBy->name : 'Sistema' }}</li>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <span class="inline-flex px-2 py-1 text-xs font-semibold bg-blue-100 text-blue-800 rounded-full">{{ $tournamentNotification->templates_used['club'] }}</span>
                </div>
                @endif
                ...
                <div>
                    ...
                    <span class="inline-flex px-2 py-1 text-xs font-semibold bg-green-100 text-green-800 rounded-full">{{ $tournamentNotification->templates_used['referee'] }}</span>
                </div>
                @endif
                ...
                <div>
                    ...
                    <span class="inline-flex px-2 py-1 text-xs font-semibold bg-yellow-100 text-yellow-800 rounded-full">{{ $tournamentNotification->templates_used['institutional'] }}</span>
                </div>
                @endif
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <h5 class="text-lg font-semibold mb-0">📧 Dettaglio Notifiche Inviate ({{ $individualNotifications->count() }})</h5>

            @if($tournamentNotification->canBeResent())
                <button type="button" class="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-colors duration-200" onclick="resendTournament()">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">🏷️ Tipo</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">📄 Oggetto</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">📊 Stato</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">⏰ Inviato</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($individualNotifications as $notification)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <code class="text-sm bg-gray-100 px-2 py-1 rounded">{{ $notification->recipient_email }}</code>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $typeClasses[$notification->recipient_type] ?? 'bg-gray-100 text-gray-800' }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900 truncate max-w-xs" title="{{ $notification->subject }}">
                                        ...
                                        <div class="flex space-x-2">
                                            ...
                                            @if($notification->attachments && count($notification->attachments) > 0)
                                                <button type="button"
                                                    ...
                                                    <span class="absolute -top-2 -right-2 bg-gray-200 text-gray-800 text-xs rounded-full px-1 min-w-4 h-4 flex items-center justify-center">{{ count($notification->attachments) }}</span>
                                                </button>
                                            @endif
                                            ...
                                            @if($notification->status === 'failed')
                                                <button type="button"
                                                        ...
                                                        onclick="resendSingle({{ $notification->id }})"
                                                        title="Reinvia">
        ...
        <div class="p-6">
            <div class="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                    <a href="{{ route('admin.tournament-notifications.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors duration-200">
                        <i class="fas fa-arrow-left mr-2"></i> Torna alla Lista
                    </a>

                    <a href="{{ route('tournaments.show', $tournamentNotification->tournament) }}" class="inline-flex items-center px-4 py-2 border border-blue-500 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors duration-200">
                ...
                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                    @if($tournamentNotification->canBeResent())
                        <button type="button" class="inline-flex items-center px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-colors duration-200" onclick="resendTournament()">
                    ...
                    <form method="POST"
                        ...
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex items-center px-4 py-2 border border-red-500 text-red-500 hover:bg-red-50 rounded-lg transition-colors duration-200">
...
<div id="contentModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-2xl shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center pb-3">
            ...
            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" onclick="closeModal('contentModal')">
        ...
        <div class="flex justify-end pt-2">
            <button type="button" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg" onclick="closeModal('contentModal')">Chiudi</button>
        </div>
    </div>
</div>
...
<div id="attachmentsModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-md shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center pb-3">
            ...
            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" onclick="closeModal('attachmentsModal')">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
            </button>
        ...
        <div class="flex justify-end pt-2">
            <button type="button" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg" onclick="closeModal('attachmentsModal')">Chiudi</button>
        </div>
    </div>
</div>
...
<div id="resendModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-md shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center pb-3">
            ...
            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" onclick="closeModal('resendModal')">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
            </button>
        ...
        <div class="flex justify-end space-x-4 pt-2">
            <button type="button" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg" onclick="closeModal('resendModal')">Annulla</button>
            <form method="POST"
