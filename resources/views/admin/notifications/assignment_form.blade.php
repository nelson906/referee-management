...
@if($referee->pivot->assigned_at)
    <div class="text-xs text-gray-400">
        Assegnato: {{ \Carbon\Carbon::parse($referee->pivot->assigned_at)->format('d/m/Y H:i') }}
    </div>
@endif
                                    </div>
                                @endforeach
...
<form method="POST"
      action="{{ ($hasExistingConvocation || (isset($documentStatus) && ($documentStatus['hasConvocation'] || $documentStatus['hasClubLetter'])))
    ...
    @csrf
                                ...
                                <div>
                                    ...
                                    <textarea name="message" id="message" rows="8" required
                                              ...
                                              placeholder="Scrivi il messaggio per la notifica...">{{ old('message', 'Si comunica l\'assegnazione degli arbitri per il torneo ' . $tournament->name . ' che si terrà ' . $tournament->start_date->format('d/m/Y') . ($tournament->start_date->format('d/m/Y') != $tournament->end_date->format('d/m/Y') ? ' - ' . $tournament->end_date->format('d/m/Y') : '') . ' presso ' . $tournament->club->name . '.
...
<!-- Gestione Allegati -->
@if($hasExistingConvocation || (isset($documentStatus) && ($documentStatus['hasConvocation'] || $documentStatus['hasClubLetter'])))
    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
        ...
        <div class="mt-3">
            ...
            <p class="text-xs text-green-600 mt-1">
                ℹ️ La lettera circolo verrà allegata solo alle email inviate al circolo organizzatore
            </p>
        </div>
    </div>
@endif
                                ...
                                @if($documentStatus['hasConvocation'] || $documentStatus['hasClubLetter'])
                                    <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                        <div class="flex items-start">
                                            <input type="checkbox" name="attach_documents" id="attach_documents" value="1" checked
                                        ...
                                        <div class="space-y-3 max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-4">
                                            @foreach($assignments as $assignment)
                                                <div class="flex items-center">
                                                    <input type="checkbox" name="recipients[]" value="{{ $assignment->user_id }}"
                                                           id="referee_{{ $assignment->id }}" checked
                                                           class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                                                    <label for="referee_{{ $assignment->id }}" class="ml-3 text-sm text-gray-900 flex-1">
                                                        <div class="flex items-center justify-between">
                                                            <div>
                                                                <span class="font-medium">{{ $assignment->user->name }}</span>
                                                                <span class="text-gray-600">({{ $assignment->role }})</span>
                                                                <div class="text-xs text-gray-500">{{ $assignment->user->email }}</div>
                                                            </div>
                                                            <div class="text-xs text-gray-400">
                                                                Assegnato {{ $assignment->assigned_at->format('d/m/Y') }}
                                                            </div>
                                                        </div>
                                                    </label>
                                                </div>
                        ...
                        @foreach($emails as $email)
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                                <label for="fixed_{{ $email->id }}" class="ml-2 text-sm text-gray-700">
...
@endif
                                ...
                                @if($institutionalEmails && $institutionalEmails->count() > 0)
                                    <div>
                                        ...
                                        <div class="space-y-4">
                                            @foreach($institutionalEmails as $category => $emails)
...
@foreach($emails as $email)
    @if(is_object($email) && isset($email->id))
        <div class="flex items-center">
            <input type="checkbox" name="institutional_emails[]" value="{{ $email->id }}"
                   id="institutional_{{ $email->id }}"
                   {{ $category === 'convocazioni' ? 'checked' : '' }}
                   class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
            <label for="institutional_{{ $email->id }}" class="ml-2 text-sm text-gray-700">
                <span class="font-medium">{{ $email->name }}</span>
                <span class="text-gray-500">({{ $email->email }})</span>
...
<div>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            const totalRecipients = recipients.length + (sendToClub ? 1 : 0) + institutionalEmails.length + additionalEmails.length;
