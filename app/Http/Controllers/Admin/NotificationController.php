...
use App\Mail\UnifiedAssignmentNotification;
use Illuminate\Support\Facades\Mail; // Aggiunto import mancante
...
class NotificationController extends Controller
{
    ...
    private function sendToinstitutionalEmail(Request $request, array $emailData)
    {
        ...
        $institutionalEmail = InstitutionalEmail::whereIn('id', $request->fixed_addresses)->get();
    ...
    private function checkAndSetExistingConvocation(Tournament $tournament)
    {
        ...
        /**
     * Create and send unified assignment notification
     */
    private function createAndSendEmail(array $emailData, string $recipientEmail, ?string $recipientName = null, bool $isClub = false)
    {
        try {
            // Crea un oggetto Notification per il mailable esistente
            $notification = new Notification([
                'subject' => $emailData['subject'],
                'message' => $emailData['message'],
                'body' => $this->formatEmailBody($emailData),
                'tournament_id' => $emailData['tournament']->id,
                'sender_id' => Auth::id(),
            ...
            if (isset($emailData['convocation']) && is_array($emailData['convocation'])) {
                ...
                foreach ($emailData['convocation'] as $attachment) {
                    if (isset($attachment['path']) && $attachment['path'] && file_exists($attachment['path'])) {
                        ...
                        // Salva l'allegato nella cartella storage per il mailable
                        ...
                        Log::info('Allegato preparato per email', [
                            'recipient' => $recipientEmail,
                            'attachment_type' => $attachment['type'],
                            'attachment_filename' => $attachment['filename'],
            ...
            // Crea e invia l'email usando il mailable esistente
            ...
            // Aggiorna lo status in caso di errore
    ...
    /**
     * Format email body combining message and tournament/assignments info
     */
    private function formatEmailBody(array $emailData): string
    {
        ...
        if (!empty($emailData['assignments']) && $emailData['assignments']->count() > 0) {
            ...
            foreach ($emailData['assignments'] as $assignment) {
                // Gestione referee-management vs gestione_arbitri
    ...
    /**
     * Get referee name handling both old and new system structure
     */
    private function getRefereeName($assignment): string
    {
        // Sistema referee-management (user-centric)
        if ($assignment->referee && $assignment->referee->user) {
            return $assignment->referee->user->name;
        }

        // Sistema gestione_arbitri (referee diretto)
        if ($assignment->user) {
    ...
    private function getTournamentAssignments(Tournament $tournament)
    {
        ...
        try {
            ...
            // Carica le relazioni manualmente per evitare problemi con eager loading
            // Gestisce sia il sistema old (referee diretto) che new (referee->user)
            $assignments->load([
                'referee',
                'referee.user',
                'referee.zone',
                'user' // Per compatibilità con il vecchio sistema
            ]);
