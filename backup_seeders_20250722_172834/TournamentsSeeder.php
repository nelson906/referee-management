...
class TournamentsSeeder extends Seeder
{
    ...
    private function createTournamentsForZone(Zone $zone): int
    {
        ...
        $tournamentsCreated += $this->createTournamentsByStatus(
            $zone, $clubs, $zoneAdmin, $tournamentTypes, 'completed', $config['completed']
        );

        // Crea tornei assegnati (passati/presenti)
        $tournamentsCreated += $this->createTournamentsByStatus(
            $zone, $clubs, $zoneAdmin, $tournamentTypes, 'assigned', $config['assigned']
        );

        // Crea tornei aperti (presenti - per raccolta disponibilità)
        $tournamentsCreated += $this->createTournamentsByStatus(
            $zone, $clubs, $zoneAdmin, $tournamentTypes, 'open', $config['open']
        );

        // Crea tornei chiusi (presenti - per assegnazioni)
        $tournamentsCreated += $this->createTournamentsByStatus(
            $zone, $clubs, $zoneAdmin, $tournamentTypes, 'closed', $config['closed']
        );

        // Crea tornei in bozza (futuri)
        $tournamentsCreated += $this->createTournamentsByStatus(
            $zone, $clubs, $zoneAdmin, $tournamentTypes, 'draft', $config['draft']
        );

        // Crea tornei programmati (futuri)
        $tournamentsCreated += $this->createTournamentsByStatus(
            $zone, $clubs, $zoneAdmin, $tournamentTypes, 'scheduled', $config['scheduled']
        );
    ...
    private function createTournamentsByStatus(
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $tournament = Tournament::create([
                ...
                'entry_fee' => $this->generateEntryFee($tournamentType),
                'prize_pool' => $this->generatePrizePool($tournamentType),
                'registration_start' => Carbon::parse($dates['start_date'])->subMonths(2)->format('Y-m-d'),
                'registration_end' => Carbon::parse($dates['start_date'])->subDays(3)->format('Y-m-d'),
                'notes' => "Torneo {$status} per testing zona {$zone->code}",
                'is_featured' => rand(0, 1) == 1,
                'weather_backup_plan' => $this->generateWeatherBackupPlan(),
                'created_at' => now(),
                'updated_at' => now(),
            ]);
    ...
    private function generateDatesForStatus(string $status): array
    {
        ...
        return match($status) {
            ...
            'scheduled' => $this->generateScheduledDates($now),
    ...
    private function createNationalTournaments(): int
    {
        ...
        foreach ($nationalTypes as $type) {
            ...
            $tournament = Tournament::create([
                ...
                'availability_deadline' => $dates['availability_deadline'],
                'status' => 'open',
                'created_by_id' => $nationalAdmin->id,
                'max_participants' => rand(100, 200),
                'entry_fee' => $this->generateEntryFee($type),
                'prize_pool' => $this->generatePrizePool($type),
                'registration_start' => Carbon::parse($dates['start_date'])->subMonths(3)->format('Y-m-d'),
                'registration_end' => Carbon::parse($dates['start_date'])->subDays(7)->format('Y-m-d'),
    ...
    private function validateTournaments(): void
    {
        ...
        // Verifica status
        $validStatuses = ['draft', 'scheduled', 'open', 'closed', 'assigned', 'completed', 'cancelled'];
